...
| エージェントのセッション境界 | **未実装**。ロールを下げても、上位ロール時に参照した内容が会話履歴に残る | ロール切替時にセッションを破棄。上位ロール用セッションとエージェント用セッションを分離し、コンテキストの持ち出しを禁止 |
...
| 7 | ファイルの境界は効くが、**会話コンテキストの境界は未実装**。同一セッション内で上位ロール時に L0 を参照すると、ロールを下げた後もその内容がエージェントの履歴に残る | ロールを下げた直後の同一セッションでは、ファイルを読まずに履歴から答えられてしまう | ロール切替でセッションを破棄する、または参照用セッションとエージェント用セッションを物理的に分離する |
